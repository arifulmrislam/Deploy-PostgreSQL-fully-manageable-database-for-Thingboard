...
I installed the Things Board community version in `The Dell Server` locally and used the `PostgreSQL` database.
In the working period, I generate some questions and try to make a note for future understanding.

```bash
ThingsBoard can use SQL or a hybrid database approach.
```
- After Installing the Thingsboard CE on the Ubuntu machine: What is necessary to back up the whole machine or only the database?

- After installing the things board on our `Ubuntu-machine` we can log thingsboard service.
```
...
```
By `*asterisk` we can copy everything from the previous folder to the new folder.
...
- Once PostgreSQL is installed we may want to create a new user or set the password for the the main user.   
  The instructions below will help to set the password for the main Postgresql user.
...
- Enter the database to check the free space of the database.

- Use `psql-console` to enter the database
```
$ psql -U postgres -h localhost
```  
- Now we will get the details of the database.
```
\l+
```
...
- check the disk space, log out from the database, and write this command.
```
$ df -h /
```
...
```
$ pg_restore -U things board -h localhost -d things board > "File name"
```
...
- After finishing the backup processing, start the things board and service.
```
$ sudo service things board start
```
## Midnight Commander

- We can use `midnight commander` to compare our things board files and our backup files. 
```
$ sudo mc
```

## File Storage
- Where are located the files with the application, like configuration files, rule chain, widget, dashboard, and so on?
- ThingsBoard can store binary content (files) in the database. Currently, it is used to store report files generated by the Generate Report Node. 
  ...
  Users can access stored files using Files or Reports widgets which is part of the Files Widgets Bundle.

- Enter the things board config file
```
$ cd /etc/thingsboard/conf
$ ll
```
- To check the user share file of things board and look at what we have here
```
...
- Basically, it's executable and some instance and installed data, etc. We have a config file where we will see the `data` folder and   here we can see the certificate and some third-party related data.
 
- Enter the extension folder, which is used for custom-developed role nodes like rule chains.
```
$ cd extensiions/
$ ll
```
...
- How to clean a Database? Or How do purge all data older than a specific date, like keep only the last six months' data?
- To remove previous data from our database, we can use `The PSQL console or Swagger`. Both have some [recommended commands](https://www.postgresqltutorial.com/postgresql-cheat-sheet/).

## `PSQL console`

- First, open psql console and connect to the thingsboard.
...
- Basically, Thingsboard stores all the data in to ts_kv table. By the below command, we can see the details of the ts_kv table. 
```
...
- If we want to delete the records from the ts_kv table using a timestamp,  `like Saturday, January 1, 2022, 11:14:40 AM to Saturday, February 1, 2022, 11:14:40 AM`,  we can do it by,
...
`Key: We should make a backup file for the database and stop the things board service before work.`

## `Swagger-ui`

- Use a browser to log in to the thingsboard [swagger service](http://localhost/swagger-ui/).
- Automatically it will open a pop-up and ask for authorizations.

<img src= "IMG/ScreenShort6.png" width=800>

- Use bearer token: bearer <past your account token>

  `Key: We don't need to stop things board service because Swagger is trying to communicate with the things board server.`
 
## `Clean events`
  
- If we want to clean our role chains, we can clean "events" from the database table. 
 ``` 
$ psql -U postgres -h localhost
```  
```  
...
## `How can we fix error writing /etc/something.conf: permission denied?`
  ...
  - Creating a Database in PostgreSQL using user name and password.
  ```
  sudo su - postgres
  psql
  postgres=#
